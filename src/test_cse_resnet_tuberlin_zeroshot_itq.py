...
import os
import pickle
from dataloaders.TUBerlin import TUBerlinDataset
import torch
...
def main():
    ...
    wandb.log({"test/tuberlin/mAP/all": mAP})
    wandb.log({"test/tuberlin/mAP/all_binary": mAP_binary})
...
def prepare_features():
    ...
    tuberlin_zero = TUBerlinDataset(split='zero', zero_version = args.zero_version, transform=transformations, aug=False)
    zero_loader = DataLoader(dataset=tuberlin_zero, batch_size=args.batch_size, shuffle=False, num_workers=3)
