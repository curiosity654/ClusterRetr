...
class QuickDrawDataset(Dataset):
    def __init__(self, split='train',
                 ...
                 version='sketch', zero_version='zeroshot1',
                 cid_mask = False, transform=None, aug=False, shuffle=False, first_n_debug=9999999, interval=1):
        ...
        if self.split == 'train':
            file_ls_file = os.path.join(self.root_dir, self.version+'_filelist_train.txt')
        elif self.split == 'val':
            file_ls_file = os.path.join(self.root_dir, self.version+'_filelist_test.txt')
        elif self.split == 'zero':
            file_ls_file = os.path.join(self.root_dir, self.version+'_filelist_zero.txt')
        else:
        ...
        self.file_ls = self.file_ls[::interval]
        self.labels = self.labels[::interval]
        self.cid2label = []

        with open(os.path.join(self.root_dir, 'cname_cid_zero.txt')) as f:
            for line in f:
                self.cid2label.append(line.split()[0])
        ...
        if cid_mask:
            cid_mask_file = os.path.join(self.root_dir, 'cid_mask.pickle')
    ...
    def __getitem__(self, idx):
        path = os.path.join(self.img_dir, self.file_ls[idx])
