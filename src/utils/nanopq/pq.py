...
class PQ(object):
    ...
    def fit(self, vecs, iter=20, seed=123):
        ...
        self.lables = np.zeros((self.M, N), dtype=np.float32)
        ...
        for m in range(self.M):
            ...
            self.codewords[m], self.lables[m] = kmeans2(vecs_sub, self.Ks, iter=iter, minit="points")
        
        for m in range(self.M):
            for n in range(N):
                self.revec[n][m * self.Ds: (m + 1) * self.Ds] = self.codewords[m][int(self.lables[m][n])]
