...
parser = argparse.ArgumentParser(description='PyTorch CSE_ResNet Model for QuickDraw Training')
...
def main():
    ...
    quickDraw_train = QuickDrawDataset(split='train', zero_version=args.zero_version, \
                                    transform=transformations, aug=True, cid_mask = True)
    train_loader = DataLoader(dataset=quickDraw_train, batch_size=args.batch_size//3, shuffle=True, num_workers=2)
    
    quickDraw_train_ext = QuickDrawDataset(split='train', version='image', zero_version=args.zero_version, \
                                         transform=transformations, aug=True, cid_mask = True)
    
    train_loader_ext = DataLoader(dataset=quickDraw_train_ext, \
                                  batch_size=args.batch_size//3*2, shuffle=True, num_workers=2)
    
    
    quickDraw_val = QuickDrawDataset(split='val', zero_version=args.zero_version, transform=transformations, aug=False)
    val_loader = DataLoader(dataset=quickDraw_val, batch_size=args.batch_size, shuffle=False, num_workers=2)
    # if args.evaluate:
