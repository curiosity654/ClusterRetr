...
from utils.tools import Proxy_Anchor
import wandb
...
def main():
    ...
    criterion_proxy = Proxy_Anchor(sz_embed=args.num_hashing, nb_classes=args.num_classes).cuda()
    ...
    for epoch in range(args.epochs):
        ...
        train(train_loader, train_loader_ext, model, criterion_train, criterion_proxy, criterion_train_kd, \
...
def train(train_loader, train_loader_ext, model, criterion, criterion_proxy, criterion_kd, \
          optimizer, epoch, model_t):

    batch_time = AverageMeter()
    losses = AverageMeter()
    losses_kd = AverageMeter()
    losses_proxy = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    for i, ((input, target, cid_mask),(input_ext, target_ext, cid_mask_ext)) in enumerate(zip(train_loader, train_loader_ext)):
        ...
        output,output_kd,features = model(input_all,tag_all,features=True)
        with torch.no_grad():
            output_t = model_t(input_all,tag_all)

        loss = criterion(output, target_all)
        loss_proxy = criterion_proxy(features, target_all)
        ...
        losses_proxy.update(loss_proxy.item(), input_all.size(0))
        ...
        train_log = {
            ...
            "train/losses_proxy": losses_proxy.avg,
        ...
        optimizer.zero_grad()
        loss_total = loss+args.sake_lambda*loss_kd+loss_proxy
